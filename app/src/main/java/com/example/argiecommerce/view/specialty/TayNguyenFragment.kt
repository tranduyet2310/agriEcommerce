...
class TayNguyenFragment : Fragment() {
    ...
    private var tayNguyenId: Long = 0L
    ...
    private fun getProducts() {
        for (subcategory in specialtyCategory!!.subCategoryList){
            if (subcategory.subcategoryName.equals(Constants.TN)){
                tayNguyenId = subcategory.id.toLong()
            }
        }

        val productApiRequest = ProductApiRequest(tayNguyenId)
    ...
    private fun setupAreaData() {
        binding.titleArea.text = requireContext().resources.getString(R.string.tay_nguyen_name)
        binding.imageArea.setImageResource(R.drawable.tay_nguyen)
        binding.areaInfo.text = requireContext().resources.getString(R.string.tay_nguyen)
    }
