...
class SubcategoryProductPagingSource(
    private val apiService: Api,
    private val productApiRequest: ProductApiRequest
) : PagingSource<Int, Product>() {

    override val keyReuseSupported: Boolean = true
    override suspend fun load(params: PagingSource.LoadParams<Int>): PagingSource.LoadResult<Int, Product> {
        return try {
            val nextPageNumber = params.key ?: 0

            val subcategoryId = productApiRequest.id
            val sortBy = productApiRequest.sortBy
            val sortDir = productApiRequest.sortDir

            val response = apiService.getProductBySubcategoryId(
                subcategoryId,
